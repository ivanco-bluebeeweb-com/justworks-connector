"""Resource handlers for Justworks Connector."""
...
async def _get_client(ctx, cid: str = ""):
    ...
    if not conn:
        return None, ActionResult.error("No active Justworks connection", code="UNAUTHORIZED")
    return JustworksClient(api_token=conn["api_token"], base_url=conn.get("base_url", "")), None
